...
######################################################################
    def urlopen(self, URL, mediaitem=0):
        ...
        if mediaitem.processor != '':
            ...
            o = urlparse(home_URL)
            #mediaitem.processor = "http://tinyurl.com/kwkwc88"
            mediaitem.processor = o.scheme+"://"+o.netloc+"/proc/youtube"
            result = self.geturl_processor(mediaitem)
        elif URL[:4] == 'http':
...
######################################################################
    def geturl_processor(self, mediaitem):
        ...
        if htmRaw=="":
            ...
            SetInfoText("Processeur: recup filtre...")
        ...
        if htmRaw[:2]=='v2':
            ...
            while exflag==False:
                ...
                if proc_args>'':
                    SetInfoText("Processeur: phase "+str(phase)+" apprentissage")
            ...
            print report
            SetInfoText("Processeur: scrapeur...")
            ...
            if match:
                ...
                for i in range(1,len(match.groups())+1):
                    ...
                    report=report+"\n v"+str(i)+": "+val
                print report
                SetInfoText("Processeur: process...")
        ...
        SetInfoText("Processor complete - chargement...")
        time.sleep(.1)
        #SetInfoText("")
