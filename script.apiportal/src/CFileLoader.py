...
#@todo: Fill parameters    
    ...
    def load(self, URL, localfile='', timeout=url_open_timeout, proxy="CACHING", \
        ...
        if (URL[:4] == 'http') or (URL[:3] == 'ftp'):
            ...
            if (os.path.exists(destfile) == True and os.path.getsize(destfile)<100): #file size less than 100 bytes
        ...
        try:
            if os.path.exists(localfile + ".old") == True and urllib.urlopen(URL).getcode() != 200:
                ...
                try:
                    ...
                    self.state =  -1 #failed         
        except IOError :#Timeout error try to read the local file
            self.state =  -1 #failed          
